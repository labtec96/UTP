...
public class Con1
{
	...
	// SQLException, ale nie będziemy go tu obsługiwać, obsługę przekażemy do
	// miejsca wywołania
	...
	void reportInfo() throws SQLException
	{
		...
		System.out.println(
				"ResultSet  TYPE_SCROLL_INSENSITIVE :" + md.supportsResultSetType(ResultSet.TYPE_SCROLL_INSENSITIVE));
		System.out.println(
				"ResultSet  TYPE_SCROLL_SENSITIVE :" + md.supportsResultSetType(ResultSet.TYPE_SCROLL_SENSITIVE));
	...
	void info(String metName)
	{
		...
		try
		{
			...
			System.out.println(infoTyp + ": " + m.invoke(md, params)); // dynamiczne
																		// wywołanie
																		// metody
		} catch (Exception exc)
		{ // Możliwe powody wyjątków: nie ma takiej metody, niewłaściwe
			// wywołanie
